...
void vga_set_external_register(const struct VGAExternalRegister *external) {
    vga_set_single_port_register(_vga_reg_port_external_output, register_const_serialize(&external->output));
    vga_set_single_port_register(_vga_reg_port_external_fc, register_const_serialize(&external->feature_control));
}