...
void vga_set_palette(const uint8_t *palette_array, uint32_t color_count) {
    for (uint32_t i = 0; i < color_count; i++) {
        ...
        vga_set_unindexed_register(_vga_reg_port_color_data, palette_array[i*3 + 0]);
        vga_set_unindexed_register(_vga_reg_port_color_data, palette_array[i*3 + 1]);
        vga_set_unindexed_register(_vga_reg_port_color_data, palette_array[i*3 + 2]);
    }
}